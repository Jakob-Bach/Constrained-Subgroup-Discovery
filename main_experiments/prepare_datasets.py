...
def prepare_datasets(data_dir: pathlib.Path) -> None:
    ...
    for dataset_name in tqdm.tqdm(dataset_overview['dataset']):
        ...
        y = (dataset['target'] == minority_class).astype(int)  # not all targets (0, 1) by default
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Retrieves datasets from PMLB, prepares them for the experiment pipeline ' +

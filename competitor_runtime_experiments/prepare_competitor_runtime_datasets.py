...
Script to save a few small datasets from PMLB (subset of main experiments) plus the "iris" dataset
from scikit-learn, so they can be used in the pipeline for competitor-runtime experiments.
...
def prepare_datasets(data_dir: pathlib.Path) -> None:
    ...
    dataset_overview = pmlb.dataset_lists.df_summary[['dataset', 'n_instances', 'n_features']]
    ...
    print("Saving scikit-learn datasets ...")
    dataset_name = 'iris'
    X, y = getattr(sklearn.datasets, 'load_' + dataset_name)(as_frame=True, return_X_y=True)
    y = (y == 0).astype(int)  # binarize multi-class target by predicting Class 0
    data_handling.save_dataset(X=X, y=y, dataset_name=dataset_name, directory=data_dir)
    dataset_overview = pd.concat([dataset_overview, pd.DataFrame({
        'dataset': [dataset_name], 'n_instances': [X.shape[0]], 'n_features': [X.shape[1]]})])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Retrieves datasets from PMLB and scikit-learn, prepares them for the' +
